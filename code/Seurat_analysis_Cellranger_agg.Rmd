...
library(vegan) # Euclidean distance function
library(lsa) # Cosine similarity calculation function
library(preprocessCore) # for quantile normalization
library(openxlsx)
library(rstatix)
library(pROC) # Calculate ROC for ML models
...
```{r}
rm(list = setdiff(ls(), c("output_dir", "data", "Blue", "SkyBlue", "Orange", "Yellow", "Vermillion")))
...
data_harm <- readRDS("../data/output/merged_samples_harmony.RDS")
...
### Figure 2C1 Cosine distance in SCTransform before harmony
...
# data2.dist = lsa::cosine(data2) # lsa library
# data2.dist = as.matrix(data2.dist) # pairwise cosine similarity of all cells in the SCTransform space
# data2.dist = 1-data2.dist # Transfer consine similarity to distance
# 
# save(data2.dist, file = paste0(sub_output_dir, "Cosine_distances_in_SCTransform_space.RData"))

```

```{r}
load(file = paste0(sub_output_dir, "Cosine_distances_in_SCTransform_space.RData"))
...
delta = c(res[,1], res[,2])
group = c(rep("High confidence", I), rep("Background", I))
```
...
ggsave(paste0(sub_output_dir, "intra_inter_group_cosine_distances.pdf"), width = 4, height = 2.8)
```

### Figure 2C2 Euclidean distance in SCTransform before harmony
...
data2.dist = as.matrix(data2.dist) # pairwise Euclidean distance of all cells in the SCTransform space

save(data2.dist, file = paste0(sub_output_dir, "Euclidean_distances_in_SCTransform_space.RData"))
...
ggsave(paste0(sub_output_dir, "intra_inter_group_Euclidean_distances.pdf"), width = 4, height = 2.8)
```


### Euclidean istance in umap before harmony
...
delta = c(res[,1], res[,2])
group = c(rep("High confidence", 100), rep("Background", 100))
```
...
### Figure 2C3 Euclidean istance in umap after harmony
...
### Euclidean distance in harmonized components
...
```{r}
data_harm <- readRDS("../data/output/merged_samples_harmony.RDS")
data2 = Embeddings(data_harm, 'harmony')
# data2 = data@assays$SCT@data # scale data or data or count? data contains technical artifact, scale data may be more biologicall relevant
...
# simple analysis - HC cells between S and L patients have higher distance than BG cells
data2.dist = vegdist(data2, method = "euclidean") # vegan library
data2.dist = as.matrix(data2.dist) # pairwise distance of all cells in the harmonized components

save(data2.dist, file = paste0(sub_output_dir, "Euclidean_distances_in_Harmonized_space.RData"))
...
ggsave(paste0(sub_output_dir, "intra_inter_group_Euclidean_distances_after_harmony.pdf"), width = 4, height = 2.8)
```

### Cosine distance in harmonized components
...
data2.dist = lsa::cosine(data2) # lsa library
data2.dist = as.matrix(data2.dist) # pairwise distance of all cells in the harmonized components
data2.dist = 1 - data2.dist
save(data2.dist, file = paste0(sub_output_dir, "Cosine_distances_in_Harmonized_space.RData"))
...
ggsave(paste0(sub_output_dir, "intra_inter_group_cosine_distances_after_harmony.pdf"), width = 4, height = 2.8)
```

### Euclidean distance in PCA components
...
```{r}
data2 = Embeddings(data, 'pca')
# data2 = data@assays$SCT@data # scale data or data or count? data contains technical artifact, scale data may be more biologicall relevant

data2 = as.matrix(data2)[, 1:2]
dim(data2)
...
save(data2.dist, file = paste0(sub_output_dir, "Euclidean_distances_in_PCA_50_components.RData"))
# randomly take N cells in each patient group (L or S)
...
ggsave(paste0(sub_output_dir, "intra_inter_group_euclidean_distances_in_PCA_50_components.pdf"), width = 4, height = 2.8)
```

### Cosine distance in PCA components
...
save(data2.dist, file = paste0(sub_output_dir, "Cosine_distances_in_PCA_50_components.RData"))
# randomly take N cells in each patient group (L or S)
N = 1000
I = 1000
res = mat.or.vec(I, 3)
...
ggsave(paste0(sub_output_dir, "intra_inter_group_cosine_distances_in_PCA_50_components.pdf"), width = 4, height = 2.8)
```

## Supplementary table 2 Number of spots per sample
...
```{r}
rm(list = setdiff(ls(), c("output_dir", "Blue", "SkyBlue", "Orange", "Yellow", "Vermillion")))
...
### Figure 4 Select the target genes for validation
...
pdf(paste0(sub_output_dir, "heatmap_decrease.pdf"), height = 2.5, width = 11)
# heatmap(t(as.matrix(plot_table)),
#         Colv=NA, Rowv = NA, cexRow = 1, cexCol = 1,
#         scale = "column")
pheatmap(
  t(as.matrix(plot_table)),
  color = colorRampPalette(brewer.pal(n = 7, name = "YlOrRd"))(100),
  cluster_rows = FALSE, cluster_col = FALSE, 
  Colv=NA, Rowv = NA, cexRow = 1, cexCol = 1,border_color = NA,
  angle_col = c("90"), scale = "column"
)
dev.off()
```
...
pdf(paste0(sub_output_dir, "heatmap_increase.pdf"),  height = 2.5, width = 11)
# heatmap(t(as.matrix(plot_table)),
#         Colv=NA, Rowv = NA, cexRow = 1, cexCol = 1,
#         scale = "column")
pheatmap(
...
# Figure 5a/b Figure S4: Gene expression violin, box, spatial plots
...
Function for merging spatial plots from different TMAs
...
```{r}
# data <- readRDS("../data/output/merged_samples.RDS")
sub_output_dir <- "../data/output/DEG/validation_gene_plots/"
if (!dir.exists(sub_output_dir)) {
  dir.create(sub_output_dir)
}
```
...
for (g in selected_genes) {
  ...
  ggsave(paste0(sub_output_dir, g, "_raw_count_spatial.pdf"), figure, height = 8, width = 21)
  ...
  # Box plots
   p <- ggplot(data = df, aes(x = group, y = expression, fill = group)) +
     labs(
       title = g,
       x = "Groups",
       y = "JUN expression level (normalized)") +
     geom_boxplot(
       # width = 0.1,
       # fill = "white",
       fatten = 1,
     ...
     stat_summary(fun.y=mean, geom="point", shape=5, size = 0.8, color="black", fill="white") +
     # geom_jitter(color="black", size=0.4, alpha=0.9) +
     scale_fill_manual(
       values = c(Orange, Yellow, SkyBlue, Blue, Vermillion)
     ) +
     scale_x_discrete(labels = c("HC-S", "BG-S", "BG-L", "HC-L")) +
    scale_y_continuous(expand = expansion(mult = c(0, 0.1))) + 
     ...
     theme(
       ...
       axis.title.x = element_text(
         size = 7, face = "plain", color = "black",
         # angle = 30, hjust = 0.5, vjust = 0.6
       ),
       axis.text.y = element_text(size = 6, color = "black"),
       axis.title.y = element_text(size = 7, face = "plain")
     )
  ggsave(paste0(sub_output_dir, g, "_normalized_box.pdf"), p, height = 2.85, width = 3)#3.85)
  # Raw count
...
```{r}
sub_output_dir <- "../data/output/GSEA/short_vs_long_PFI_in_HC/"
if (!dir.exists(sub_output_dir)){
  dir.create(sub_output_dir)
}
```
...
```{r}
sub_output_dir <- "../data/output/GSEA/Heatmap/"
if (!dir.exists(sub_output_dir)){
  dir.create(sub_output_dir)
}
```
...
gsea_HC <- NULL
for (s in c("H")) {
  tmp <- read.csv(paste0( "../data/output/GSEA/short_vs_long_PFI_in_HC/deg_single_cell_logFC_manual_", s, "_dif.csv"))
  if (is.null(gsea_HC)){
    gsea_HC <- tmp
  } else {
...
gsea_BG <- NULL
for (s in c("H")) {
  tmp <- read.csv(paste0( "../data/output/GSEA/short_vs_long_PFI_in_BG/deg_single_cell_logFC_manual_", s, "_dif.csv"))
  if (is.null(gsea_BG)){
    gsea_BG <- tmp
  } else {
...
```{r}
pathway_merge <- rbind.data.frame(pathway_merge, plot_table)
```
...
#### Merged pathways (H + C2:CP)

```{r}
pathways_in_C2_CP <- unique(c(
  unlist(msigdbr(species = "human", category = "C2", subcategory = "CP:BIOCARTA")$gs_name),
  unlist(msigdbr(species = "human", category = "C2", subcategory = "CP:KEGG")$gs_name),
  unlist(msigdbr(species = "human", category = "C2", subcategory = "CP:PID")$gs_name),
  unlist(msigdbr(species = "human", category = "C2", subcategory = "CP:REACTOME")$gs_name),
  unlist(msigdbr(species = "human", category = "C2", subcategory = "CP:WIKIPATHWAYS")$gs_name)
))
pathways_in_C2_CP <- gsub("_", " ", pathways_in_C2_CP)
pathway_merge_reorder <- pathway_merge %>% 
  filter(pathway %in% pathways_in_C2_CP | startsWith(pathway, "HALLMARK")) %>% 
  dplyr::mutate(
    category = dplyr::case_when(
      pathway %in% c("HALLMARK G2M CHECKPOINT", "KEGG CELL CYCLE",
                     "REACTOME APC C MEDIATED DEGRADATION OF CELL CYCLE PROTEINS",
                     "REACTOME CELL CYCLE", "REACTOME CELL CYCLE CHECKPOINTS",
                     "REACTOME CELL CYCLE MITOTIC", "WP CELL CYCLE",
                     "WP G1 TO S CELL CYCLE CONTROL", "HALLMARK E2F TARGETS",
                     "HALLMARK MYC TARGETS V1")
      ~ "Cell cycle control and replication",
      pathway %in% c("HALLMARK GLYCOLYSIS", "HALLMARK OXIDATIVE PHOSPHORYLATION",
                     "HALLMARK ADIPOGENESIS", "HALLMARK MTORC1 SIGNALING")
      ~ "Cellular metabolism",
      pathway %in% c("REACTOME ACTIVATION OF ATR IN RESPONSE TO REPLICATION STRESS",
                     ...
                     "PID FANCONI PATHWAY") 
      ~ "DNA repair including HR",
      pathway %in% c("HALLMARK COMPLEMENT", "HALLMARK INTERFERON ALPHA RESPONSE")
      ~ "Innate immune\nresponse",
      pathway %in% c("HALLMARK APOPTOSIS", "HALLMARK P53 PATHWAY",
                     "HALLMARK TNFA SIGNALING VIA NFKB","HALLMARK UV RESPONSE UP")
      ~ "Stress response",
      TRUE ~ ""
      )
    ) %>% 
  # Format pathway names for figure
  dplyr::mutate(
    pathway_db = dplyr::case_when(
      startsWith(pathway, "HALLMARK") ~ "Hallmark",
      startsWith(pathway, "PID") ~ "PID",
      startsWith(pathway, "REACTOME") ~ "Reactome",
      startsWith(pathway, "WP") ~ "WikiPathways",
      startsWith(pathway, "KEGG") ~ "KEGG",
      TRUE ~ ""
      )
    ) %>% 
  dplyr::mutate(
    pathway_name = gsub("^[[:alpha:]]+\ ", "", pathway)
  ) %>% 
  dplyr::mutate(
    pathway_print = paste0(pathway_name, " (", pathway_db, ")")
  )

write.csv(pathway_merge_reorder, paste0(sub_output_dir, "reordered_merged_pathway_H_C2CP_for_heatmap.csv"), row.names = FALSE)
```
...
```{r}
pathway_merge_reorder <- read.csv(paste0(sub_output_dir, "reordered_merged_pathway_H_C2CP_for_heatmap.csv"))
hm.palette <- colorRampPalette(rev(brewer.pal(9, 'RdYlBu')), space='Lab')
pathway_merge_reorder$group <- factor(pathway_merge_reorder$group, levels = c("High confidence", "Background"))
ggplot(
  pathway_merge_reorder,
  aes(x = group, y = pathway_print, fill=NES)) + 
  geom_tile()  +
  # scale_fill_gradientn(colours = hm.palette(100)) +
  # scale_fill_gradient2(
  #   high = Orange,
  ...
  facet_grid(
    rows = vars(category), scales = "free", space="free_y"
  )
ggsave(
  paste0(sub_output_dir, "short_vs_long_PFI_hallmark_H_C2CP_pathway_heatmap.pdf"),
  width = 7,
  height = 7)
```
...
# Figure 3: Predicting the L/S state using HC/BG cells

```{r}
rm(list = setdiff(ls(), c("output_dir", "Blue", "SkyBlue", "Orange", "Yellow", "Vermillion")))
sub_output_dir <- paste0("../data/output/prediction/")
if (!dir.exists(sub_output_dir)) {
  dir.create(sub_output_dir)
}
```
...
for(i in 1:length(patient)){
  ...
  pred_HC = as.data.frame(h2o.predict(fit_HC, data_HC_test))
  pred_HC_merge[[i]] <- pred_HC
  ...
  pred_BG = as.data.frame(h2o.predict(fit_BG, data_BG_test))
  pred_BG_merge[[i]] <- pred_BG
...
pred_HC_merge <- Reduce(rbind.data.frame, pred_HC_merge)
pred_BG_merge <- Reduce(rbind.data.frame, pred_BG_merge)

save(pred_HC_merge, pred_BG_merge,file = paste0(sub_output_dir, "res_gbm_predict_results.RData"))

write.csv(res, paste0(sub_output_dir, "res_gbm_predict_results.csv"), row.names = FALSE)
...
```{r}
res <- read.csv(paste0(sub_output_dir, "res_gbm_predict_results.csv"))
res_bootstrap <- read.csv(paste0(sub_output_dir, "res_bootstrap_accuracy_gbm.csv"))
...
write.csv(
  plot_table,
  paste0(sub_output_dir, "res_bootstrap_accuracy_confidence_interval_GBM.csv"),
  row.names = FALSE
)
ggplot(
  ...
  labs(
    y = "Accuracy from gradient boosting machine",
    fill = ""
  ) +
  theme_classic() +
  theme(
...
ggplot(
  filter(plot_table, PFI == "S"),
  aes(x = patient, y = accuracy, fill = area)
  ) +
  geom_boxplot(outlier.size = 0.3, fatten = 1, lwd=0.3) +
  scale_fill_manual(
    values = c(Orange, Yellow),
    labels = c("Short PFI high confidence", "Short PFI background")
  ) +
...
ggsave(paste0(sub_output_dir, "prediction_accuracy_box_plot_short_PFI_gbm.pdf"), width = 4.8, height = 2)
ggplot(
 filter(plot_table, PFI == "L"),
  aes(x = patient, y = accuracy, fill = area, )
  ) +
...
ggsave(paste0(sub_output_dir, "prediction_accuracy_box_plot_long_PFI_gbm.pdf"), width = 4.8, height = 2)
```
...
ggplot(
  ...
  geom_violin() +
  #geom_boxplot(outlier.size = 0.7, fatten = 1, lwd=0.2) +
  scale_fill_manual(
    values = c(Orange, Yellow),
    labels = c("Short PFI high confidence", "Short PFI background")
  ) +
...
ggsave(paste0(sub_output_dir, "prediction_accuracy_violin_plot_short_PFI_gbm.pdf"), width = 4.8, height = 2)
ggplot(
 filter(plot_table, PFI == "L"),
  aes(x = patient, y = accuracy, fill = area, )
  ) +
...
ggsave(paste0(sub_output_dir, "prediction_accuracy_violin_plot_long_PFI_gbm.pdf"), width = 4.8, height = 2)
```

### Confidence interval signif per sample

```{r}
plot_table <- read.csv(paste0(sub_output_dir, "res_bootstrap_accuracy_confidence_interval_gbm.csv"))
sig <- NULL
...
save(pred_HC_merge, pred_BG_merge,file = paste0(sub_output_dir, "res_GLM_predict_results.RData"))

write.csv(res, paste0(sub_output_dir, "res_GLM_predict_results.csv"))
# res$PFI = unlist(lapply(patient, function(x) unique(group3[which(group4==x)])[1]))
# 
# write.xlsx(res, paste0(sub_output_dir, "res_gbm.xlsx"), rowNames = T)
```
...
```{r}
res <- read.csv(paste0(sub_output_dir, "res_GLM_predict_results.csv"))
res_bootstrap <- read.csv(paste0(sub_output_dir, "res_bootstrap_accuracy_GLM.csv"))
plot_table <- res_bootstrap %>% 
  dplyr::group_by(patient, area) %>% 
  summarise(
    mean = mean(accuracy),
...
write.csv(
  plot_table,
  paste0(sub_output_dir, "res_bootstrap_accuracy_confidence_interval_GLM.csv"),
  row.names = FALSE
)
...
ggplot(
  ...
  labs(
    y = "Accuracy from generalized linear model",
    fill = ""
  ) +
  theme_classic() +
  theme(
...
ggsave(paste0(sub_output_dir, "prediction_accuracy_box_plot_short_PFI_GLM.pdf"), width = 4.8, height = 2)
ggplot(
 filter(plot_table, PFI == "L"),
  aes(x = patient, y = accuracy, fill = area, )
  ) +
...
ggsave(paste0(sub_output_dir, "prediction_accuracy_box_plot_long_PFI_GLM.pdf"), width = 4.8, height = 2)
```
...
ggsave(paste0(sub_output_dir, "prediction_accuracy_violin_plot_short_PFI_GLM.pdf"), width = 4.8, height = 2)
ggplot(
 filter(plot_table, PFI == "L"),
  aes(x = patient, y = accuracy, fill = area, )
  ) +
...
ggsave(paste0(sub_output_dir, "prediction_accuracy_violin_plot_long_PFI_GLM.pdf"), width = 4.8, height = 2)
```

### Confidence interval signif per sample

```{r}
plot_table <- read.csv(paste0(sub_output_dir, "res_bootstrap_accuracy_confidence_interval_GLM.csv"))
sig <- NULL
for (s in unique(plot_table$patient)) {
  tmp_data <- plot_table %>% 
    filter(patient == s)
...
save(pred_HC_merge, pred_BG_merge,file = paste0(sub_output_dir, "res_deeplearning_predict_results.RData"))

write.csv(
  res,
  paste0(sub_output_dir, "res_deeplearning_predict_results.csv"),
  row.names = FALSE
)
# res$PFI = unlist(lapply(patient, function(x) unique(group3[which(group4==x)])[1]))
# 
...
```{r}
res <- read.csv(paste0(sub_output_dir, "res_deeplearning_predict_results.csv"))
res_bootstrap <- read.csv(paste0(sub_output_dir, "res_bootstrap_accuracy_deeplearning.csv"))
plot_table <- res_bootstrap %>% 
  dplyr::group_by(patient, area) %>% 
  summarise(
    mean = mean(accuracy),
...
write.csv(
  plot_table,
  paste0(sub_output_dir, "res_bootstrap_accuracy_confidence_interval_deeplearning.csv"),
  row.names = FALSE
)
...
ggplot(
  ...
  labs(
    y = "Accuracy from deep neural network",
    fill = ""
  ) +
  theme_classic() +
  theme(
...
ggsave(paste0(sub_output_dir, "prediction_accuracy_box_plot_short_PFI_deeplearning.pdf"), width = 4.8, height = 2)
ggplot(
 filter(plot_table, PFI == "L"),
  aes(x = patient, y = accuracy, fill = area, )
  ) +
...
ggsave(paste0(sub_output_dir, "prediction_accuracy_box_plot_long_PFI_deeplearning.pdf"), width = 4.8, height = 2)
```
...
ggsave(paste0(sub_output_dir, "prediction_accuracy_violin_plot_short_PFI_deeplearning.pdf"), width = 4.8, height = 2)
ggplot(
 filter(plot_table, PFI == "L"),
  aes(x = patient, y = accuracy, fill = area, )
  ) +
...
ggsave(paste0(sub_output_dir, "prediction_accuracy_violin_plot_long_PFI_deeplearning.pdf"), width = 4.8, height = 2)
```

### Confidence interval signif per sample

```{r}
plot_table <- read.csv(paste0(sub_output_dir, "res_bootstrap_accuracy_confidence_interval_deeplearning.csv"))
sig <- NULL
for (s in unique(plot_table$patient)) {
  tmp_data <- plot_table %>% 
    filter(patient == s)
...
```{r}
plot_table <- read.csv("../data/output/prediction/res_bootstrap_accuracy_confidence_interval_deeplearning.csv")
test_table <- plot_table %>% 
  select(patient, area, mean) %>% 
  pivot_wider(names_from = area, values_from = mean)
t.test(
...
save(pred_HC_merge, pred_BG_merge,file = paste0(sub_output_dir, "res_randomForest_predict_results.RData"))

write.csv(
  res,
  paste0(sub_output_dir, "res_randomForest_predict_results.csv"),
  row.names = FALSE
)
# res$PFI = unlist(lapply(patient, function(x) unique(group3[which(group4==x)])[1]))
# 
...
```{r}
res <- read.csv(paste0(sub_output_dir, "res_randomForest_predict_results.csv"))
res_bootstrap <- read.csv(paste0(sub_output_dir, "res_bootstrap_accuracy_randomForest.csv"))
plot_table <- res_bootstrap %>% 
  dplyr::group_by(patient, area) %>% 
  summarise(
    mean = mean(accuracy),
...
write.csv(
  plot_table,
  paste0(sub_output_dir, "res_bootstrap_accuracy_confidence_interval_randomForest.csv"),
  row.names = FALSE
)
...
ggsave(paste0(sub_output_dir, "prediction_accuracy_box_plot_short_PFI_randomForest.pdf"), width = 4.8, height = 2)
ggplot(
 filter(plot_table, PFI == "L"),
  aes(x = patient, y = accuracy, fill = area, )
  ) +
...
ggsave(paste0(sub_output_dir, "prediction_accuracy_box_plot_long_PFI_randomForest.pdf"), width = 4.8, height = 2)
```
...
ggsave(paste0(sub_output_dir, "prediction_accuracy_violin_plot_short_PFI_randomForest.pdf"), width = 4.8, height = 2)
ggplot(
 filter(plot_table, PFI == "L"),
  aes(x = patient, y = accuracy, fill = area, )
  ) +
...
ggsave(paste0(sub_output_dir, "prediction_accuracy_violin_plot_long_PFI_randomForest.pdf"), width = 4.8, height = 2)
```

### Confidence interval signif per sample

```{r}
plot_table <- read.csv(paste0(sub_output_dir, "res_bootstrap_accuracy_confidence_interval_randomForest.csv"))
sig <- NULL
for (s in unique(plot_table$patient)) {
  tmp_data <- plot_table %>% 
    filter(patient == s)
...
```{r}
plot_table <- read.csv("../data/output/prediction/res_bootstrap_accuracy_confidence_interval_randomForest.csv")
test_table <- plot_table %>% 
  select(patient, area, mean) %>% 
  pivot_wider(names_from = area, values_from = mean)
t.test(
...
## Summary figure for all the accuracies

### Box plot

```{r}
gbm <- read.csv("../data/output/prediction/res_bootstrap_accuracy_confidence_interval_GBM.csv")
glm <- read.csv("../data/output/prediction/res_bootstrap_accuracy_confidence_interval_GLM.csv")
dl <- read.csv("../data/output/prediction/res_bootstrap_accuracy_confidence_interval_deeplearning.csv")
rf <- read.csv("../data/output/prediction/res_bootstrap_accuracy_confidence_interval_randomForest.csv")

merge <- mutate(gbm, model = "GBM") %>% 
  rbind.data.frame(mutate(glm, model = "GLM")) %>% 
  rbind.data.frame(mutate(dl, model = "Deep Learning")) %>%
  rbind.data.frame(mutate(rf, model = "Random Forest"))
merge$area <- factor(merge$area, levels = c("HC", "BG"))
p <- ggplot(
  filter(merge),
  aes(x = patient, y = mean, fill = area)
  ) +
...
ggsave(
  paste0(sub_output_dir, "summary_for_all_models.pdf"),
  p,
  width = 9, height = 5
  )
```
...
```{r}
test_table_merge <- NULL
for (i in c("GBM", "GLM", "deeplearning", "randomForest")) {
  plot_table <- read.csv(paste0("../data/output/prediction/res_bootstrap_accuracy_confidence_interval_", i, ".csv"))
  test_table <- plot_table %>% 
    select(patient, area, mean) %>% 
    pivot_wider(names_from = area, values_from = mean) %>% 
    mutate(model = i)
  
  if (is.null(test_table_merge)) {
    test_table_merge <- test_table
  } else {
    test_table_merge <- rbind.data.frame(test_table_merge, test_table)
  }
}
...
```{r}
# true <- read.csv("../data/output/prediction/res_gbm_predict_results.csv")
model <- c("gbm", "GLM", "deeplearning", "randomForest")
for (m in model) {
  rm(pred_BG_merge, pred_HC_merge)
  # load(paste0("../data/output/prediction/res_", m, "_predict_results_with_probability.RData"))
  # pred_BG_merge$true <- true[true$area == "BG", ]$true
  # pred_HC_merge$true <- true[true$area == "HC", ]$true
  pred <- read.csv(paste0("../data/output/prediction/res_", m, "_predict_results.csv"))
  pred_BG_merge <- filter(pred, area == "BG")
  pred_HC_merge <- filter(pred, area == "HC")
  pred_HC_merge$true <- factor(pred_HC_merge$true, levels = c("L", "S"))
  pred_BG_merge$true <- factor(pred_BG_merge$true, levels = c("L", "S"))
  pdf(
    paste0("../data/output/prediction/res_", m, "_prediction_ROC.pdf"),
    width = 4,
    height = 4
  )
  model <- switch(
  ...
  rocobj1 <- plot.roc(
    pred_HC_merge$true, pred_HC_merge$S,
    direction = "<",
    main=paste0(model),
    percent=TRUE,
    col=Blue
  ...
  rocobj2 <- lines.roc(
    pred_BG_merge$true, pred_BG_merge$S,
    print.auc = TRUE,
    direction = "<",
    percent=TRUE,
    col=SkyBlue
  ...
  text(48, 23, labels=paste("P value", format.pval(testobj$p.value)), adj=c(0, .5), cex = 0.85)
  auc_BG <- as.numeric(auc(rocobj2))
  text(48, 29, labels=paste("AUC (BG):", round(auc_BG, 2), "%"), adj=c(0, .5), col = SkyBlue, cex = 0.85)
  auc_HC <- as.numeric(auc(rocobj1))
  text(48, 35, labels=paste("AUC (HC):", round(auc_HC, 2), "%"), adj=c(0, .5), col =  Blue, cex = 0.85)
  legend("bottomright", legend=c("HC", "BG"), col=c(Blue, SkyBlue), lwd=2, y.intersp = 0.6)
  dev.off()
}
```
### END of Figure 3b
...
```{r}
sub_output_dir <- paste0("../data/output/C5_C7_abundance/")
if (!dir.exists(sub_output_dir)){
  dir.create(sub_output_dir)
}
```
